...
class Deploy(object):
    ...
    def __call__(self, *args, **kwargs):
        ...
        try:
            ...
            alarm(ColoredText().color('r').style('b')(f'Exception: ') + f'{e}')
...
class Pipeline(object):
    ...
    def __call__(self, *args, kwargs: Optional[Union[Dict[str, Any], Tuple[Dict[str, Any]]]] = None):
        ...
        if isinstance(kwargs, dict):
            ...
            if len(kwargs) != len(self.run_flow):
                raise ValueError(
                    f'Number of the given tuples of keyword arguments is different from length of callables in the runflow:\n'
                    f'kwargs: {len(kwargs)}, runflow: {len(self.run_flow)}'
                    )
        else:
            raise ValueError(f'The "kwargs" argument must be either a tuple or a dictionary, {type(kwargs)} is given')
    ...
    def run_flow(self, steps):
        if not isinstance(steps, Iterable) and not isinstance(steps, Callable) and not isinstance(steps, Deploy):
...
class NumberedDict(UserDict):
    ...
    def append(self, *args, **kwargs):
        if args and kwargs:
            raise TypeError('append() takes either positional or keyword arguments but both were given')
        elif args:
            if len(args) == 1:
                ...
                raise TypeError(f'append() takes 1 or 2 positional arguments but {len(args)} were given')
        elif kwargs:
