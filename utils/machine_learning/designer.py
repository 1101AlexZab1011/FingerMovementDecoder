...
class ModelDesign(AbstractDesign, Pipeline):
    ...
    def __call__(self, inputs: Optional[Union[np.ndarray, tf.Tensor]] = None, kwargs: Optional[Union[dict[str, Any], tuple[dict[str, Any]]]] = None) -> tf.Tensor:
...
class ParallelDesign(AbstractDesign):
    def __init__(self, *args: Union[None, ModelDesign, tf.keras.layers.Layer], activation: Union[str, tf.keras.layers.Activation] = None):
        ...
        self._run_flow = (arg for arg in args if arg is not None) if self.activation is None else (*(arg for arg in args if arg is not None), self.activation)
    ...
    def __call__(self, inputs: Optional[Union[np.ndarray, tf.Tensor]] = None) -> tf.Tensor:
        ...
        return self.activation(tf.keras.layers.Add()(vars)) if self.activation is not None else tf.keras.layers.Add()(vars)
    ...
    def replace(self, design: ModelDesign, index: int):
        ...
        self._run_flow = self._parralels if self.activation is None else (*self._parralels, self.activation)
...
class LayerDesign(AbstractDesign, Deploy):
    def __init__(self, layer_transformator: Callable, *args, **kwargs):
        ...
        self._run_flow = (arg for arg in [*args, *list(kwargs.values())] if issubclass(type(arg), (tf.keras.layers.Layer, AbstractDesign)))
