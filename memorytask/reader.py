...
if __name__ == "__main__":
    ...
    parser.add_argument('-sd', '--source-dir', type=str,
                        default=os.path.join(os.getcwd(), 'Source', 'Data'),
    ...
    if os.path.isdir(info_path):
        ...
        if n_info_files != n_subjects:
            raise ValueError(
                f'Number of info files ({n_info_files}) '
                f'and number of subjects ({n_subjects}) are inconsistent'
            )
    ...
    for subject, subject_info in zip(os.listdir(source_dir), info):
        ...
        for stimulus in os.listdir(subject_dir):
            ...
            try:
                ...
                alarm(
                    f'Epochs for {subject} {stimulus} can not be read '
                    f'due to {type(err)}\n{err}\n',
                    True
                )
                continue
