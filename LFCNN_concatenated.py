...
from LFCNN_decoder import Predictions, SpatialParameters, TemporalParameters,\
    ...
    save_parameters, plot_patterns, plot_waveforms, get_order

if __name__ == '__main__':
    mpl.use('agg')
    parser = argparse.ArgumentParser(
        description='A script for applying the neural network "LFCNN" '
        'to the epoched data from gradiometers related to events for '
    ...
    parser.add_argument('-c', '--cases', type=str, nargs='+',
                        default=['LI', 'LM', 'RI', 'RM'],
                        help='Cases to consider (must match '
                        'epochs file names for the respective classes)')
    parser.add_argument('-cmb', '--combine-cases', type=str, nargs='+',
                        default=None,
                        help='Cases to consider (must be the number of strings '
                        'in which classes to combine are written separated '
                        'by a space, indices corresponds to order '
                        'of "--cases" parameter)')
    ...
    patterns_fig.savefig(
        os.path.join(
            patterns_pics_path,
            f'{subject_name}_{classification_name_formatted}.png'
        )
    )
